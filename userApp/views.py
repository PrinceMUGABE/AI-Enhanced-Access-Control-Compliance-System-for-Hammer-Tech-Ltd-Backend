...
def login_with_otp_request(request):
    """Step 1: Request OTP for login - Use email instead of emp_number"""
    ...
    try:
        ...
        if not user.check_password(password):
            ...
            ActivityLogger.log_authentication(
                user=user,
                activity='login_otp_request',
                description=f'Failed: Invalid password for {email}',
                request=request,
                response=response,
                is_success=False,
                start_time=start_time
        ...
        otp = generate_otp()
        store_otp(user.email, otp, expiry_seconds=30)
    ...
    try:
        ...
        if not is_valid:
            ...
            ActivityLogger.log_authentication(
                user=user,
                activity='login_otp_verify',
                description=f'Failed: Invalid OTP - {message}',
                request=request,
                response=response,
                is_success=False,
                start_time=start_time
...
def get_user_activity_logs(request, user_id):
    ...
    try:
        ...
        # Check permissions
        if not requesting_user.is_admin and not requesting_user.is_hr:
            try:
