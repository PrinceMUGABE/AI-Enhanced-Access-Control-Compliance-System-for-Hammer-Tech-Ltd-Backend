...
def get_all_incidents(request):
    ...
    try:
        user = request.user
        print(f"User {user.email} with role {user.role} attempted to access all incidents")
        
        # Check if user is admin
        if not user.is_admin or user.role != 'security_analyst' and user.role != 'admin':
            print(f"User {user.email} with role {user.role} is not authorized to access all incidents")
            return Response(
                {"success": False, "error": "Only administrators can view all incidents."},
                status=status.HTTP_403_FORBIDDEN
            )
        
        print(f"Admin {user.email} fetching all incidents")
