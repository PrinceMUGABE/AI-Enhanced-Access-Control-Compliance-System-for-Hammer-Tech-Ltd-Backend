...
class IncidentUtils:
    ...
    def track_incident_progress(incident):
        ...
        if incident.sla_due_date and now() > incident.sla_due_date:
            if not incident.sla_violated:
                ...
                return {
                    ...
                    'overdue_hours': (now() - incident.sla_due_date).total_seconds() / 3600
                }
        
        # Check if incident is approaching SLA deadline (within 1 hour)
        if incident.sla_due_date and not incident.sla_violated:
            time_to_deadline = (incident.sla_due_date - now()).total_seconds() / 3600
    ...
    @staticmethod
    def get_incident_tracking_data(incident):
        """Get detailed tracking data for an incident"""
        from django.utils.timezone import now
        ...
        # Time since creation
        data['time_since_creation'] = (current_time - incident.created_at).total_seconds() / 3600
        
        # Time since assignment
        if incident.assigned_at:
            data['time_since_assignment'] = (current_time - incident.assigned_at).total_seconds() / 3600
        
        # Time to SLA deadline
        if incident.sla_due_date:
            if current_time < incident.sla_due_date:
                data['time_to_sla_deadline'] = (incident.sla_due_date - current_time).total_seconds() / 3600
            else:
                data['time_over_sla'] = (current_time - incident.sla_due_date).total_seconds() / 3600
        ...
        data['current_progress']['percentage'] = status_progress.get(incident.status, 0)
        data['current_progress']['status'] = incident.status
        
        # Next expected action based on status
        next_actions = {
            'pending': 'Assign to a user for investigation',
            'investigating': 'Determine root cause and impact',
            'assigned': 'Begin working on resolution',
            'in_progress': 'Complete resolution steps',
            'resolved': 'Review and close incident',
            'escalated': 'Higher priority handling required'
        }
        
        data['current_progress']['next_action'] = next_actions.get(incident.status, 'Monitor progress')
    ...
    def get_incident_statistics(user=None, timeframe_days=30):
        ...
        Args:
            user: User to filter incidents by (if provided)
        ...
        # Calculate cutoff date for trends
        ...
        severity_distribution = list(queryset.values('severity').annotate(
            count=Count('id')
        ).order_by('-count'))
        ...
        # Department distribution (if user can see)
        department_distribution = []
        if not user or user.is_admin or user.is_hr:
            department_distribution = list(queryset.filter(
        ...
        # SLA compliance
        incidents_with_sla = queryset.filter(sla_due_date__isnull=False)
        sla_violations = incidents_with_sla.filter(sla_violated=True).count()
        total_with_sla = incidents_with_sla.count()
        sla_compliance_rate = round(
            ((total_with_sla - sla_violations) / total_with_sla * 100) if total_with_sla > 0 else 100, 1
        )
        
        # Overdue incidents (past SLA but not marked as violated yet)
        ...
        # Danger zone logs
        danger_zone_logs = queryset.filter(danger_zone=True).count()
        
        # Average risk score
        avg_risk_score = round(queryset.aggregate(
            avg_risk=Avg('risk_score')
        )['avg_risk'] or 0, 1)
        
        # Recent trends (last timeframe_days)
        recent_incidents = recent_queryset.count()
        previous_cutoff = cutoff_date - timedelta(days=timeframe_days)
        previous_incidents = queryset.filter(
            created_at__gte=previous_cutoff,
            created_at__lt=cutoff_date
        ).count()
        
        # Calculate growth rate
        incident_growth_rate = 0
        if previous_incidents > 0:
            incident_growth_rate = round(
                ((recent_incidents - previous_incidents) / previous_incidents * 100), 1
            )
        ...
        statistics = {
            ...
            'danger_zone_logs': danger_zone_logs,
    ...
    def check_sla_compliance():
        ...
        Returns list of incidents with SLA violations
        """
        from django.utils.timezone import now
        ...
        for incident in violated_incidents:
            # Update violation flag if not set
            if not incident.sla_violated:
                incident.sla_violated = True
                incident.save()
...
class DangerZoneAnalyzer:
    ...
    def analyze_logs_for_danger(timeframe_hours=24, risk_threshold=60):
        ...
        Returns logs with risk scores above threshold
        """
        ...
        ).order_by('-timestamp')[:1000]  # Limit to 1000 logs for performance
        ...
        return danger_logs[:100]  # Return top 100
    ...
    def get_ip_context(ip_address):
        ...
        return {
            ...
            'first_seen': ip_activities.order_by('timestamp').first().timestamp if ip_activities.exists() else None,
            'last_seen': ip_activities.order_by('-timestamp').first().timestamp if ip_activities.exists() else None
        }
    ...
    def get_recommended_action(log, risk_score):
        ...
        if risk_score >= 80:
            return "Immediate investigation required. Consider temporary account suspension."
        elif risk_score >= 70:
            return "High priority investigation. Assign to security team."
        elif risk_score >= 60:
            return "Monitor closely. Consider additional authentication requirements."
        else:
            return "Review and monitor if pattern continues."
    ...
    def get_danger_zone_summary(timeframe_hours=24):
        ...
        for log in danger_logs:
            ...
            user_risk[user_email] += log['risk_score']
